...
function LogoImg({ src, domain, name, className }) {
  ...
  if (idx >= srcs.length)
    return (
      <div
        className={`${className} bg-gray-100 dark:bg-gray-700 rounded-xl flex items-center justify-center text-3xl font-extrabold text-gray-400 dark:text-gray-500`}
      >
        {name.charAt(0)}
      </div>
    );
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                {company.history.map((row, idx) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-3 text-yellow-600 dark:text-yellow-400 font-medium">
                      {row.avg}
                    </td>
                    <td className="px-6 py-3 text-green-600 dark:text-green-400 font-medium">
                      {row.highest}
                    </td>
                  </tr>
                ))}
