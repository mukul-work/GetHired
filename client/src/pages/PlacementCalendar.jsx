...
export default function PlacementCalendar() {
  ...
  const filtered = EVENTS.filter((e) => {
    if (filterBranch !== "All" && !e.branches.includes(filterBranch)) return false;
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors duration-200">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: "Total Events", value: EVENTS.length, color: "text-gray-900", icon: "🗓️" },
            { label: "Drives", value: EVENTS.filter((e) => e.type === "Drive").length, color: "text-green-600", icon: "🚀" },
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <span className={`text-[10px] uppercase tracking-wide font-bold px-2.5 py-1 rounded-full ${TAG_BADGE[event.tag]}`}>
                        {event.tag === "dream" ? "⭐ Dream" : event.tag === "mass" ? "👥 Mass" : "🏢 Normal"}
                      </span>
                    </div>
